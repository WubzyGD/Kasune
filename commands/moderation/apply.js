...
module.exports = {
    ...
    async execute(message, msg, args, cmd, prefix, mention, client) {
        ...
        if (!ma.enabled && !message.author.id === '330547934951112705') {return message.channel.send("Staff applications are not currently open at this time! We'll announce when we're looking for mods, so keep your eyes peeled!");}
        ...
        let name = await ask(mesg, "**⚙༄✦-ྀ࿐ Q1 •** What is your name (optional) and Discord username (with tag)?", 120000, true); if (!name) {return clearDM();}
        ...
        let agep = await ask(mesg, "**⚙༄✦-ྀ࿐ Q2 •** How old are you and what are your preferred pronouns?", 90000, true); if (!agep) {return clearDM();}
        if (agep.length > 150) {clearDM(); return dmch.send(":sweat_smile: let's keep your answer a little shorter than that, please.");}

        let hobbies = await ask(mesg, "**⚙༄✦-ྀ࿐ Q3 •**  Do you have any special hobbies? (Optional, if you choose not to, put '**N/A**') (Please be brief)", 180000, true); if (!hobbies) {return clearDM();}
        if (hobbies.length > 750) {clearDM(); return dmch.send("Heya! Please send just a quick list of your hobbies!");}

        let tz = await ask(mesg, "**⚙༄✦-ྀ࿐ Q4 •** What is your time zone? (EX: CST, EST) ", 60000, true); if (!tz) {return clearDM();}
        if (tz.length > 150) {clearDM(); return dmch.send("That's way too long to be a timezone...");}

        let experience = await ask(mesg, "**⚙༄✦-ྀ࿐ Q5 •** Do you have any Moderation experience? If so, what kind of community/communities did you moderate, what position(s) did you have? If you have loads of experience, please specify your most notable and lengthy positions.", 240000, true); if (!experience) {return clearDM();}
        ...
        let contrib = await ask(mesg, "**⚙༄✦-ྀ࿐ Q6 •** If you were to be accepted into our team, what special contributions can you bring to the server and team? How would the community benefit from your role?", 240000, true); if (!contrib) {return clearDM();}
        ...
        let time = await ask(mesg, "**⚙༄✦-ྀ࿐ Q7 •** What days are you active on Discord, and how much time do you typically spend in the server? (Use mostly numbers and quick answers, like \"I'm active whenever I'm not at school/work, and I usually spend 3 or 4 hours a day in the server.\" Yes, you can use that if it applies to you)", 120000, true); if (!time) {return clearDM();}
        if (time.length > 500) {clearDM(); return dmch.send("Oi! I said be brief!");}

        let warn = await ask(mesg, "**⚙༄✦-ྀ࿐ Q8 •** What do you consider to be a **warnable offense**? *(aka giving someone a warning through a bot or verbal)*", 120000, true); if (!warn) {return clearDM();}
        if (warn.length > 750) {clearDM(); return dmch.send("Just like... a couple examples is fine...");}

        let mute = await ask(mesg, "**⚙༄✦-ྀ࿐ Q9 •** What do you consider to be a **mute-able offense**? *(aka preventing someone from being able to speak for a given duration of time)*", 120000, true); if (!mute) {return clearDM();}
        if (mute.length > 750) {clearDM(); return dmch.send("Just like... a couple examples is fine...");}

        let ban = await ask(mesg, "**⚙༄✦-ྀ࿐ Q10 •** What do you consider to be a **bannable offense**? *(aka permanently removing someone from the server)*", 120000, true); if (!ban) {return clearDM();}
        if (ban.length > 750) {clearDM(); return dmch.send("Just like... a couple examples is fine...");}

        let traits = await ask(mesg, "**⚙༄✦-ྀ࿐ Q11 •** What are some of your **strengths** and **weaknesses**? You don't have to spoil your deepest darkest secret, and if you're not comfortable with sharing, then say **N/A**, but this makes selecting you more difficult as we won't know you as well.", 240000, true); if (!traits) {return clearDM();}
        ...
        let notes = await ask(mesg, "**⚙༄✦-ྀ࿐ Q12 •** Anything else you'd like to briefly add? (**N/A** if not)", 240000, true); if (!notes) {return clearDM();}
        ...
        try {
            let mch = await client.fetchWebhook("870888287227752468", "p-1RLJqMPWnr5hi1jYH_G83ZN2NUOS-VgK5kh7bn1NgLZKn2besYmncfn1pvAXTOz_Nz");
            mch.send("<@&867134094362017792>", {
                username: "Kit | Staff Applications",
